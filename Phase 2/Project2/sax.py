...
sax = SAX(world_size, alphabet_size, 10**(-5))
fi = open("dataset.txt", "r+")
fo = open("sax_dataset_converted.txt", 'w+')
fo.write(str(alphabet_size))
fo.write(" ")
fo.write(str(world_size))
fo.write(" ")
fo.write(str(margin))
fo.write("\n")
s1 = fi.readline()
...
for s5 in l2:
    if(s5 == []):
        continue
    (sax_convert, tmp) = sax.to_letter_rep(s5)
    fo.write (str(sax_convert))
    fo.write("\n")
fo.close()
fi.close()
print "<<<SAX CONVERTING DONE>>>"
print "Powerd by AZINAA TEAM"
